...
public class ConFunciones {
			...
			while (numjug != numsec) {
				...
				try {
					...
					if (numjug > numsec && numjug < 101 && numjug > 0) {
						...
						resumen(numsec, contnumjug, numalto, numbajo);
						break;
	...
	// Funciones (se deben poner fuera del main)

	public static void resumen(int numsec, int contnumjug, int numalto, int numbajo) {
	...
	public static int modoj(int modo, Scanner sc) {
		System.out.println(
		...
		while (modo != 1 && modo != 2) {
			...
			try {
				...
				if (modo == 1) {
					...
					System.out.println("Opción no válida. Debes elegir un número dentro del rango");
